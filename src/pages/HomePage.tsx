        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {games.map((game) => (
            <Link key={game.path} to={game.path}>
              <div className="group relative bg-[#1E2328] rounded-lg p-6 hover:bg-[#282D34] transition-all duration-200">
                <div className="flex items-start justify-between mb-4">
                  <div className="w-12 h-12 flex items-center justify-center">
                    <game.icon className={`w-8 h-8 ${game.iconColor}`} />
                  </div>
                  <ArrowUpRight className="w-5 h-5 text-[#3B82F6] opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
                <h3 className="text-white text-lg font-semibold mb-2">{game.title}</h3>
                <p className="text-[#94A3B8] text-sm">{game.description}</p>
              </div>
            </Link>
          ))}
        </div>
